...
class ELG(nn.Module):
    ...
    def forward(self, cur_room_emb, max_length, choose_token_method="sample"):
        ...
        for i in range(max_length):
            ...
            #output -> (seq_len,batch_size,room_emb_size)
            #这里是一个一个单词输入，所以seq_len=1
        ...
        #spoken_token.size=(batch_size,max_sent_len)    
...
class ELU(nn.Module):
    ...
    def _encode_message(self, message):
        ...
        # msg_embs -> (batch_size*history, message len, voc_emb_size)
        # hx -> (1,batch_size,room_emb_size)
    ...
    def forward(self, env_emb, message, choose_room_method="sample", room_num_differ=None):
        #shape从(a,b)到(a,1,b)
        if len(message.shape) == 2: message = message.unsqueeze(1)
        room_prob = []
        hx = self._encode_message(message)
        #矩阵相乘，第一维相同，第一个的第三维和第二个的第二维相同
        #           batch_size         room_emb_size   
        #.squeeze()删除单条目的维度
        #res -> (batch_size, room_num)
        res = torch.bmm(env_emb, hx.permute(1, 2, 0)).squeeze()
        # make sure that the scores for each padding gate is 0
        # 如果环境中房间数量不同，保证被填充的房间是不可能被猜测到的
        score_mask = torch.zeros_like(res)
        if room_num_differ is not None:
            room_nums_list = room_num_differ.tolist()
            for i, cur_room_num in enumerate(room_nums_list):
                if cur_room_num < res.shape[1]:
                    res[i, int(cur_room_num):] = -torch.inf        
                    score_mask[i, int(cur_room_num):] = 1          
        scores = self.softmax(res)
        assert room_num_differ is None or torch.sum(scores[score_mask == 1]) == 0
